...
def read_barcodes_new(barcodes_file: Path) -> dict:
    """
    ...
    barcode_df.set_index('barcode',inplace=True)
    barcode_dict=barcode_df.to_dict('index')
...
def make_barseq_directories(runner) -> None:
    ...
    :return:
    """
    error_message =f"Barseq Error: {runner.experiment} directory already exists. Delete or rename {runner.experiment}"
    results_folder = Path(runner.experiment)
